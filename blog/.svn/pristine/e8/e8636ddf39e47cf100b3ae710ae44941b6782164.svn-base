...
public class BlogAction extends ActionSupport implements ModelDriven<Blog> {
	...
	@Action(value = "mhcxofallBlog", results = { @Result(name = "input", location = "index.jsp") })
	public String mhcxofallBlog() {
		User user = userService.selectbyId((Integer) ServletActionContext.getRequest().getSession().getAttribute("id"));
	Set set = user.getBlogtypes();
		SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");
		String a=  df.format(blog.getInsertTime());
		 String b=   df.format(blog.getUpdateTime());
		 Blogtype blogtype1=blogTypeService.selectbyId(blog.getBlogTypeId());
		
	List<Blog>blogs= blogService.mhcxofall(blog.getTitle(),a,b,blogtype1);
		/* List<Blog> bList=blogService.mhcx(blog.getTitle(),blogtype1);*/
